...
for index, row in carts.iterrows():
    b.write(f"DB::table('carts')->insert([" + f"'total' => '{row['total']}', 'Customer_id' => '{row['Customer_id']}' " + "]);\n")
b.close()

b = open('cart_items.txt', 'w')
for index, row in cart_items.iterrows():
    b.write(f"DB::table('cart_items')->insert([" + f"'quantity' => '{row['quantity']}','price' => '{row['price']}','total'=>'{row['total']}' ,'Cart_id' => '{row['Cart_id']}', 'Item_id' => '{row['Item_id']}' " + "]);\n")
b.close()
...
for index, row in employees.iterrows():
    b.write(f"DB::table('employees')->insert([" + f"'salary' => '{row['salary']}', 'User_id' => '{row['User_id']}', 'Business_id' => '{row['Business_id']}'" + "]);\n")
b.close()
...
for index, row in orders.iterrows():
    b.write(f"DB::table('orders')->insert([" + f"'total' => '{row['total']}', 'description' => '{row['description']}' ,'Customer_id' => '{row['Customer_id']}'" + "]);\n")
b.close()

b = open('order_items.txt', 'w')
for index, row in order_items.iterrows():
    b.write(f"DB::table('order_items')->insert([" + f"'quantity' => '{row['quantity']}','price' => '{row['price']}', 'total' => '{row['total']}', 'Order_id' => '{row['Order_id']}', 'Item_id' => '{row['Item_id']}' " + "]);\n")
b.close()

b = open('users.txt', 'w')
for index, row in users.iterrows():
    b.write(f"DB::table('users')->insert([" + f"'name' => '{row['name']}', 'age' => '{row['age']}', 'phone' => '{row['phone']}', 'email' => '{row['email']}', 'password' => '{row['password']}', 'Gender_id' => '{row['Gender_id']}' " + "]);\n")
b.close()